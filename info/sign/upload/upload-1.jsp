...
<%
 ...
 if (isMultipart) {                                                                                            // multipart로 전송 되었을 경우
  File temporaryDir = new File("./temp");                                                 //업로드 된 파일의 임시 저장 폴더를 설정
  String realDir = config.getServletContext().getRealPath("/info/sign/slides");                  //톰켓의 전체 경로를 가져오고 upload라는 폴더를 만들고 거기다
  ...
  while(iter.hasNext()){
   ...
   if(fileItem.isFormField()){                                                                               //업로드도니 파일이 text형태인지 다른 형태인지 체크
    ...
    String yymmdd = str.subString(2,2)+str.subString(5,2)+str.subString(8,2);

    paramNames.add(str);
    //out.println(paramNames.size());
    ...
    if(fileItem.getSize()>0){                                                                                 //파일이 업로드 되었나 안되었나 체크 size>0이면 업로드 성공
     ...
     out.println(paramNames.get(0) + paramNames.get(1)+"<br/>");
